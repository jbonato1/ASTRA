...
parser.add_argument('-fr','--data_fr',type=int, help='Data fraction for training')
...
for item in items:
    ...
    if os.path.isfile(path_to_item) and file_extension == '.tif' and filename[-2:]!='nf' and filename[-2:]!='nh' and int(item[6:9])<26 :
...
##### fraction of data:
fr = out_im.shape[0]-(out_im.shape[0]*args.data_fr)//100
print('From',out_im.shape[0],'to: ',fr)
out_im = out_im[:fr,:,:,:]
label = label[:fr,:,:,:]
...
weights_str='/media/DATA/jbonato/segm_project/weights/dataset_th/'+args.model+str(args.data_fr)+'_'+test_folder_str+'.pt'
