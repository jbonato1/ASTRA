...
for frac in fraction:
    ...
    for item in items:
        ...
        if os.path.isfile(path_to_item) and file_extension == '.tif' and filename[-2:]!='nh' and int(item[6:9])<26 :
    ...
    weights_str='/media/DATA/jbonato/segm_project/weights/fraction/'+args.model+test_folder_str+'_'+str(frac)+'.pt'
    print('SAVE',weights_str)
    ...
    model,loss_val,_,_ = train_model(model, optimizer_ft, exp_lr_scheduler, num_epochs=args.epochs-3,dataloaders=dataloaders,device=device,single_loss=single_loss,use_visdom=False)
    if args.model == 'dense_up':
        ct=0
        #for child in model.children():
        ...
        exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=200, gamma=0.1)
        model,loss_val,_,_ = train_model(model, optimizer_ft, exp_lr_scheduler,num_epochs=3,best_loss=loss_val,dataloaders=dataloaders,device=device,single_loss=single_loss)
