...
for jj in range(1,26):
    ...
    if jj !=20:
        ...
        #model.load_state_dict(torch.load('/media/DATA/jbonato/segm_project/nips_weights/'+args.model+test_folder_str1+'pret.pt'))
        max_min = dict_area[str(jj)]
        model.load_state_dict(torch.load('/media/DATA/jbonato/segm_project/weights/no_pproc/'+args.model+test_folder_str1+'.pt'))
        print('/media/DATA/jbonato/segm_project/weights/no_pproc'+args.model+test_folder_str1+'.pt')
        ...
        image_to_plot = np.median(stack,axis=0)         
       ...
       ################################### 
        ...
        plt.savefig('/media/DATA/jbonato/segm_project/U-net/test/'+args.model+'no_pp'+str(jj)+'.png')
        plt.close()
        #save DNN output
        with h5py.File('/media/DATA/jbonato/segm_project/U-net/Results/no_pproc/SMALL_'+test_folder_str+'.hdf5','w') as f:
