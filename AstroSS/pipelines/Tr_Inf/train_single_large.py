...
import time
import copy
from aug_samples_single48 import * 
...
parser.add_argument('-m','--model',default ='dense_up',choices=['UNet','Unet_nest_5','nestedUNetUp5_dense','dense_up'], help='model name (default:Unet_nest_5)')

parser.add_argument('-d','--data', metavar='DIR', default='/media/DATA/jbonato/segm_project/set_large/train_single',help='path to dataset')
...
counter = 0
counter2 = 0
im = np.empty((48,48),dtype = np.float32 )
for item in items:
    ...
    if os.path.isfile(path_to_item) and file_extension == '.tif' and filename[-2:]!='nf':
        ...
        dset= h5py.File(filename+'nf.hdf5','r') 
        soma_mask =  np.asarray(dset['Values_soma'])

        back = np.ones((48,48),dtype=np.int64)-soma_mask
        back[back<0]=0
...
def fun (i):
    ...
    im_cont = np.empty((13,1,N,M))
    mask_cont = np.empty((13,2,N,M))

    sample = np.empty((13,3,N,M))

    im_cont, mask_cont =  aug(im = out_im[i,0,:,:], mask = label[i,:,:,:])#np.dstack((,label[i,1,:,:])))
...
dataloaders = {
    'train': DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=args.workers),
    'val': DataLoader(val_set, batch_size=batch_size, shuffle=True, num_workers=args.workers)
}
...
weights_str='/media/DATA/jbonato/segm_project/weights/'+args.model+test_folder_str+'large.pt'#gen_pret.pt'
...
#exp_lr_scheduler = lr_scheduler.MultiStepLR(optimizer_ft, milestones=[8,30], gamma=0.1)

model,loss_val = train_model(model, optimizer_ft, exp_lr_scheduler, num_epochs=args.epochs-6,dataloaders=dataloaders,device=device,single_loss=single_loss)#

ct=0
if args.model == 'dense_up':
    for child in model.module.children():
        if ct>1 and ct<5:
            print('Activating child', ct)
            for params in child.parameters():
...
if args.model == 'dense_up':
    ...
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=220, gamma=0.1)
    model,loss_val = train_model(model, optimizer_ft, exp_lr_scheduler, num_epochs=3,dataloaders=dataloaders,device=device,single_loss=single_loss,use_visdom=False)
...
del model,out_im,out_im_val,label,label_val,train_set,val_set,qq
