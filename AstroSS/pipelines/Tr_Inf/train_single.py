...
for item in items:
    ...
    if os.path.isfile(path_to_item) and file_extension == '.tif' and filename[-2:]!='nh' and filename[-2:]!='nf':
        ...
        dset= h5py.File(filename+'.hdf5','r')###ATTENZIONE 
...
weights_str='/media/DATA/jbonato/segm_project/weights/'+args.model+test_folder_str+'sb_jb_nth.pt'#gen_pret.pt'
...
model,loss_val,loss_tr,loss_valid = train_model(model, optimizer_ft, exp_lr_scheduler, num_epochs=args.epochs-3,dataloaders=dataloaders,device=device,single_loss=single_loss)#args.epochs-3
...
if args.model == 'dense_up':
    ...
    model,loss_val,loss_tr_f,loss_val_f = train_model(model, optimizer_ft, exp_lr_scheduler,num_epochs=3,best_loss=loss_val,dataloaders=dataloaders,device=device,single_loss=single_loss)
...
#mod
loss_tr = np.append(loss_tr,loss_tr_f)

loss_valid = np.append(loss_valid,loss_val_f)


loss_f = np.hstack((loss_tr[:,np.newaxis],loss_valid[:,np.newaxis]))
#save 
np.savetxt('/media/DATA/jbonato/segm_project/U-net/Results/loss_val/'+test_folder_str+'.csv', loss_f, delimiter=',')
