...
parser.add_argument('-m','--model',default ='dense_up',choices=['Unet_nest_5','nestedUNetUp5_dense','dense_up'], help='model name (default:Unet_nest_5)')
...
N=430
max_min = np.loadtxt('/media/DATA/jbonato/segm_project/U-net/data3.csv',delimiter=',')
...
for jj in range(5,6):
    ...
    if jj !=20 and jj!=23:
        ...
        model.load_state_dict(torch.load('/media/DATA/jbonato/segm_project/weights/'+args.model+test_folder_str1+'large.pt'))
        print('FOV',test_folder_str,args.model+test_folder_str1)
        #collect stack to analyze
        stack_dir = '/media/DATA/jbonato/segm_project/set_large/'+test_folder_str+'/'
        ...
        #################MASK
        set_dir = '/media/DATA/jbonato/segm_project/set_large/dataset/'
        items = os.listdir(set_dir)
        
        for i in items:
            if i=='SMALL_'+test_folder_str+'_nf.hdf5':
                mask_test = np.empty((N,N))
        
                dset= h5py.File(set_dir+i,'r') 
                mask_test=  np.asarray(dset['Values_soma'])
        ...
        if len(coord_l)!=0:
            ...
            with h5py.File('/media/DATA/jbonato/segm_project/U-net/Results/dense_up/SMALL_'+test_folder_str1+'_LARGE.hdf5','w') as f:
                        dset2 = f.create_dataset('Values_soma',data=Res_1[:,:,0])
            
            
#             #plot the develop of segmentation and correlation
#             fig, ax = plt.subplots(figsize=(30, 15), nrows=1, ncols=3)
#             ax[0].imshow(image_to_plot)
#             ax[1].imshow(mask_test) 
#             ax[2].imshow(Res_1[:,:,0])

#             plt.savefig('/media/DATA/jbonato/segm_project/U-net/test/'+args.model+'_NSmall_bl_gpu'+str(jj)+'LARGE.png')
#             plt.close()
