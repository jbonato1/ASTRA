...
for jj in range(1,26):
    ...
    if jj !=20:
        ...
        #model.load_state_dict(torch.load('/media/DATA/jbonato/segm_project/weights/'+args.model+test_folder_str1+'sb_jb_nth.pt'))
        
        model.load_state_dict(torch.load('/media/DATA/jbonato/segm_project/weights/'+args.model+test_folder_str1+'noise_red_2.pt'))
        ...
        stack_dir = '/media/DATA/jbonato/segm_project/set_SNR/set_'+noise+'/'+test_folder_str+'/'
        ...
        #set_dir = '/media/DATA/jbonato/segm_project/mask_compl/'
...
#                 plt.close()
            ...
            #save DNN output
            with h5py.File('/media/DATA/jbonato/segm_project/U-net/Results/Res_SNR/SMALL_'+test_folder_str+'_n'+noise+'aug_rm.hdf5','w') as f:
                dset = f.create_dataset('Values',data=Res_1[:,:,0])
                dset2 = f.create_dataset('Values_soma',data=Res_1[:,:,1])
            #'/media/DATA/jbonato/segm_project/U-net/Results/dense_up/SMALL_'+test_folder_str+'nth.hdf5'
            #'/media/DATA/jbonato/segm_project/U-net/Results/Res_SNR/SMALL_'+test_folder_str+'_n'+noise+'.hdf5'
#             with h5py.File('/media/DATA/jbonato/segm_project/U-net/Results/dense_up/SMALL_'+test_folder_str+'nth.hdf5','w') as f:
#                 dset = f.create_dataset('Values',data=Res_1[:,:,0])
#                 dset2 = f.create_dataset('Values_soma',data=Res_1[:,:,1])

            ref = np.zeros((256,256,3))
            ref[:,:,0] = mask_test[:,:,0]
            ref[:,:,1] = mask_test[:,:,1]
        else:
            Res_1[:,:,:]=0
            with h5py.File('/media/DATA/jbonato/segm_project/U-net/Results/Res_SNR/SMALL_'+test_folder_str+'_n'+noise+'.hdf5','w') as f:
                dset = f.create_dataset('Values',data=Res_1[:,:,0])
                dset2 = f.create_dataset('Values_soma',data=Res_1[:,:,1])
