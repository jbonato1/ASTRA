...
for jj in range(1,26):
    ...
    if jj !=20:
        ...
        print('/media/DATA/jbonato/segm_project/weights/no_pproc/'+args.model+test_folder_str1+'nf_enh.pt')
        ...
        if len(coord_l)!=0:
            ...
            image_stack, filt_imageL = filter_.save_im(Filt=False)
            image_set = image_stack[:,0,:,:]
            image_set = image_set[:,np.newaxis,:,:]
            #new start
            imageL_set = image_to_plot.astype(np.float32)
            ...
            #save DNN output
            with h5py.File('/media/DATA/jbonato/segm_project/U-net/Results/no_pproc/SMALL_'+test_folder_str+'_no_filt.hdf5','w') as f:
            ...
            ax[1].imshow(Res_1) 
            plt.savefig('/media/DATA/jbonato/segm_project/U-net/test/'+args.model+'_Small'+str(jj)+'no_filt.png')
            plt.close()
