...
import time
import copy
from aug_samples_single192 import * 
...
parser.add_argument('-d','--data', metavar='DIR', default='/media/DATA/jbonato/segm_project/set/train_single_car',help='path to dataset')
...
counter = 0
counter2 = 0
im = np.empty((192,192),dtype = np.float32 )
for item in items:
    ...
    if os.path.isfile(path_to_item) and file_extension == '.tif':
        ...
        back = np.ones((192,192),dtype=np.int64)-proc_mask-soma_mask
        back[back<0]=0
...
model.load_state_dict(torch.load('/media/DATA/jbonato/segm_project/weights/gen_pret.pt'))
...
weights_str='/media/DATA/jbonato/segm_project/weights/'+args.model+test_folder_str+'_256.pt'
...
model,loss_val = train_model(model, optimizer_ft, exp_lr_scheduler, num_epochs=args.epochs,dataloaders=dataloaders,device=device,single_loss=single_loss)
...
del qq, model,out_im,label,out_im_val,label_val
