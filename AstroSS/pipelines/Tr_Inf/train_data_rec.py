...
parser.add_argument('-iter','--iteration',type=int, help='Iteration of training')
...
weights_str='/media/DATA/jbonato/segm_project/weights/data_iter/'+args.model+'_iter_'+str(args.iteration)+'_'+test_folder_str+'.pt'
...
model,loss_val = train_model(model, optimizer_ft, exp_lr_scheduler, num_epochs=args.epochs-3,dataloaders=dataloaders,device=device,single_loss=single_loss,use_visdom=False)#args.epochs-3
ct=0
if args.model == 'dense_up':
